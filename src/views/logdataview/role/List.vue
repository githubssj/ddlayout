...
export default {
  ...
  methods: {
    loadTableData() {
      ...
      //从网络上加载数据
      this.$nsneohttp.get('/localmock/logdataview/role.json').then(httpdata => {
        //获得列头
    ...
    create() {
      this.$router.replace({ path: "./roleedit", query: { id: "" } });
    },
    edit(param) {
      console.log(param);
      this.$router.replace({ path: "./roleedit", query: { id: param.row.id } });
    },
    remove(param) {
      console.log(param);
