  ...
  <div class="nsneo-inner-root">
    <div class="nsneo-inner-header">
      <Breadcrumb class="nsneo-module-breadcrumb">
        ...
        <BreadcrumbItem>
          <Icon type="logo-buffer"></Icon>图表组件列表
        </BreadcrumbItem>
      </Breadcrumb>
...
export default {
  ...
  methods: {
    loadTableData() {
      ...
      //从网络上加载数据
      this.$nsneohttp.get('/localmock/logdataview/chartctrl.json').then(httpdata => {
        //获得列头
    ...
    renderTableColumn(tableColumns){
      ...
      //对 img 列进行重绘
      tableColumns.forEach(col => {
        if (col.key && col.key == "img") {
          col.render = (h, params) => {
            const row = params.row;
            const imgsrc = row.img;
            //console.log(imgsrc);
            return h('img', {
              attrs:{
                src: imgsrc,
                class:'nsneo-table-imgcol'
              }
    ...
    create() {
      this.$router.replace({ path: "./chartctrledit", query: { id: "" } });
    },
    edit(param) {
      console.log(param);
      this.$router.replace({ path: "./chartctrledit", query: { id: param.row.id } });
    },
    remove(param) {
      console.log(param);
