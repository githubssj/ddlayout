  ...
  <div class="nsneo-inner-root">
    <div class="nsneo-inner-header">
      <Breadcrumb class="nsneo-module-breadcrumb">
        <BreadcrumbItem>
          <Icon type="ios-home-outline"></Icon>调度规则管理
        </BreadcrumbItem>
        <BreadcrumbItem>
          <Icon type="logo-buffer"></Icon>调度规则列表
        </BreadcrumbItem>
      </Breadcrumb>
...
export default {
  ...
  methods: {
    loadTableData() {
      ...
      //从网络上加载数据
      this.$nsneohttp.get('/localmock/logdataview/dispatch.json').then(httpdata => {
        //获得列头
    ...
    create() {
      this.$router.replace({ path: "./dispatchedit", query: { id: "" } });
    },
    edit(param) {
      console.log(param);
      this.$router.replace({ path: "./dispatchedit", query: { id: param.row.id } });
    },
    remove(param) {
      console.log(param);
